...
> **Desvio aplicado na verificação (commit 487a098):** o bloco de sensores inteiro
> (CPU→SISTEMA) e o separador foram deslocados **+110px** em relação às coordenadas
> abaixo — o rótulo da CPU em y2230 colidia com o logo "NieR:Automata" da arte, na
> faixa quase transparente do fade. Os scripts commitados são a fonte da verdade.
...
function StaticText($name,$x,$y,$size,$color,$font='Segoe UI') {
    # Default hardcoded no parametro: em PS, $font e $FONT sao a MESMA variavel
    # (case-insensitive) — um fallback "if (-not $font) { $font = $FONT }" atribui
    # null a si mesmo e serializa <Font></Font> vazio.
